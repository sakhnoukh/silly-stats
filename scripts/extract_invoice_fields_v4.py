...
class InvoiceExtractor:
    ...
    def _looks_bad_issuer(self, val: Optional[str]) -> bool:
        ...
        if re.fullmatch(r'(bill|from|vendor|seller|supplier)', s, re.I):
            return True

        if re.search(
            r'(spanish to english|onlinedoc|onlinedoctranslator|translation|translated by|^original$|^copy$)',
            s,
            re.I,
        ):
            return True
    ...
    def _extract_company_name_ocr(self, file_path: Optional[str]) -> Optional[str]:
        ...
        candidates = []
        for idx, row in enumerate(search_rows[:15]):
        ...
        if ocr_total:
            if not result["total_amount"]:
                ...
                try:
                    ...
                    # Lower threshold than before because many real invoices had regex
                    # selecting a subtotal / fee instead of the final total.
