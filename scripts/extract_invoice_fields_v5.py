...
class InvoiceExtractor:
            ...
            if match:
                ...
                if tail_match and re.search(r'[A-Za-z]', val):
                    ...
                    if not (len(tail_val) == 4 and 1900 <= int(tail_val) <= 2099):
                        val = f"{val} {tail_val}"
                # Strip trailing pure-alpha words (e.g. "039-0002-486391 Leganes" → "039-0002-486391")
    ...
    def extract_recipient_name(self, text: str) -> Optional[str]:
        ...
        same_line_patterns = [
            ...
            r'(?:cliente|comprador|destinatario|acheteur|facturer\s*[àa])\s*[:\-]?\s*([^\n]{2,120})',
        ]
        ...
        for ln in lines[:60]:
            ...
            if not re.fullmatch(r'(?:[A-ZÁÉÍÓÚÑÜ]{4,15})(?:\s+[A-ZÁÉÍÓÚÑÜ]{4,15}){1,3}', stripped):
                continue
    ...
    def _post_ocr_cleanup(text: str) -> str:
        ...
        # Common OCR confusions in name contexts (pipe → l)
    ...
    def _looks_bad_issuer(self, val: Optional[str]) -> bool:
        ...
        if re.fullmatch(
            r'(?:[A-ZÁÉÍÓÚÑÜ]{4,15})(?:\s+[A-ZÁÉÍÓÚÑÜ]{4,15}){1,2}',
            s,
        ):
    ...
    def _extract_invoice_number_ocr(self, file_path: Optional[str]) -> Optional[str]:
        ...
        anchor_re = re.compile(
            r'(invoice|inv|factura|facture|reference|ref|booking|confirmation|folio)\s*(?:no|number|num|n[oº°]?|id|#)?',
            re.I,
        )
        ...
        for i, row in enumerate(rows[:40]):
            ...
            m = re.search(
                r'(?:invoice|inv|factura|facture|reference|ref|booking|confirmation|folio)\s*(?:no|number|num|n[oº°]?|id|#)?\s*[:#\-]?\s*([A-Z0-9][A-Z0-9/_\-. ]{2,40})',
                text,
                re.I,
            )
            if m:
    ...
    def extract(self, invoice_text: str, file_path: Optional[str] = None, **kwargs) -> Dict:
        ...
        # ── Email-domain fallback for issuer (last resort) ───────────────
