...
def _valid_invoice_number(val):
    if not val:
        return False
    if len(val) < 3 or len(val) > 24:
        return False
    if not re.search(r"\d", val):
        return False
    if re.match(r"^(amount|date|number|total|page|client)$", val, re.IGNORECASE):
        return False
...
def main():
    ...
    print(f"✓ {json_path}")
    print(f"✓ {cov_path}")
    print("\nDone. Review results/invoice_extractions.csv to verify quality.")
