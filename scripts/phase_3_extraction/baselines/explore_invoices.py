...
print(f"Empty text: {(invoices['raw_text'].str.strip() == '').sum()}")
print(f"Avg word count: {invoices['raw_text'].str.split().str.len().mean():.1f}")
print()
...
OUTPUT_DIR = PROJ_ROOT / "data" / "exploration"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
out_path = OUTPUT_DIR / "invoice_samples.txt"
...
for label, pattern in keywords.items():
    count = invoices["raw_text"].str.contains(pattern, case=False, regex=True).sum()
...
for fmt, pat in date_patterns.items():
    count = invoices["raw_text"].str.contains(pat, regex=True).sum()
    print(f"  {fmt:<20}: in {count} documents")

# Show actual date examples
all_dates = []
for _, row in invoices.iterrows():
    found = re.findall(r"\b(?:\d{1,2}[/-]\d{1,2}[/-]\d{2,4}|[A-Za-z]{3,9}\s+\d{1,2},?\s+\d{4})\b", row["raw_text"])
...
for fmt, pat in amount_patterns.items():
    matches = invoices["raw_text"].str.contains(pat, regex=True).sum()
    print(f"  {fmt:<20}: in {matches} documents")

all_amounts = []
for _, row in invoices.iterrows():
    found = re.findall(r"\$[\d,]+\.?\d*|\b\d{1,6}\.\d{2}\b", row["raw_text"])
...
noise_indicators = {
    "non-ASCII chars":       invoices["raw_text"].apply(lambda t: bool(re.search(r"[^\x00-\x7F]", t))).sum(),
    "short words ratio>0.5": invoices["raw_text"].apply(
        lambda t: (sum(1 for w in t.split() if len(w) <= 2) / max(len(t.split()), 1)) > 0.5
    ).sum(),
    "avg word length < 3":   invoices["raw_text"].apply(
