"""Quick AFTER-only eval on synthetic invoices (tests/invoices)."""
...
INVOICES_DIR = Path(__file__).resolve().parent.parent / "tests" / "invoices"
GT = json.load(open(INVOICES_DIR.parent / "ground_truth.json", encoding="utf-8"))
...
total_c = total_n = 0
per_field_c = {f: 0 for f in FIELDS}
per_field_n = {f: 0 for f in FIELDS}
